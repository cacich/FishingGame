...
function documentFor(locations) {
  ...
  const lines = [
    ...
    '',
    '整站批量產圖可把 12 條提示依 row-major 排成 3×4 規格表、使用純洋紅鍵色背景，再以 `tools/split-sprite-sheet.py` 按 id 名單切格、去背並縮進 96×56 RGBA 畫布。這能穩住同站色盤與像素密度，但仍要人工確認格數、順序及魚頭朝右；工具不會猜測錯位的格子。',
    ''
  ];
