...
function documentFor(locations) {
  ...
  const lines = [
    ...
    '',
    '整站批量產圖可把 12 條提示依 row-major 排成 3×4 規格表、使用精確 `#FF00FF` 純洋紅鍵色背景，再以 `tools/split-sprite-sheet.py` 按 id 名單切格、去背、保留最大連通主體並縮進 96×56 RGBA 畫布。提示詞必須禁止輪廓外光暈／半透明 bloom，發光限制在輪廓內或緊貼輪廓的實色像素。這能穩住同站色盤與像素密度，但仍要人工確認格數、順序、單一主體及魚頭朝右；工具不會猜測錯位的格子，也不能救被大片光暈污染的鍵色背景。',
    '',
