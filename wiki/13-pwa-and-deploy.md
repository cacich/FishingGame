...
`sw.js` 開頭目前是 `const VERSION = 'v18';`。`v11` 在十六個釣點背景與縮圖之外，加入當時 370 張 AI 產圖精靈的預快取；`v12`～`v17` 依序處理索引、朝向、canvas 重繪與船上角色序列。`v18` 新增朱楓天守、雪見狐湯的 4 張場景圖與 47 張精靈，正式資產總量變為十八站、417 個可釣項目。這類圖片與腳本都是 cache-first，改動後一定要再升版。
