...
`sw.js` 開頭目前是 `const VERSION = 'v13';`。`v11` 在十六個釣點背景與縮圖之外，加入全部 370 張 AI 產圖精靈的預快取；`v12` 則使修正精靈索引初始化時機的 `js/pixel.js` 重新進入快取，避免既有 PWA 繼續使用會黑屏的 v11 腳本；`v13` 使其餘十五張地圖的 301 張魚類鏡像精靈重新下載。這類圖片與腳本都是 cache-first，改動後一定要再升版。
