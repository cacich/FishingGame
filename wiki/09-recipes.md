...
6. **產生並接上完整正式美術。** 新釣點不能只靠 `scene.terrain`／`buildFish()`／`JUNK_MAPS` 看起來能玩就算完成；這三者必須保留做失敗備援，但正式交付還要同時具備：
   ...
   - 用 `tools/split-sprite-sheet.py` 依 id 名單切格、去鍵色並縮進 96×56；它會把與透明背景相連的鍵色混色帶改成最近主體色，再重著色最外圈的高亮紫／桃紅 glow，**不是只提高 `--key-tolerance` 把輪廓削掉**。主體跨格時用 `--component-cells`；必要時再用 `tools/flip-sprites.py` 修正魚頭方向。
   - 驗收必須同時通過：檔案數 = `loc.fish.length`、每張為 96×56 RGBA、同時有透明與不透明像素、安全留白、魚頭朝右。另把成品排在**遊戲同款深色底**逐張看，最外圈不得有洋紅鍵色殘留或亮紫／桃紅光邊；腳本檢查需確認近鍵色外緣像素與高亮紫外緣像素都是 0。合法鬚、蒸氣與裝飾可能分離，連通元件數只能輔助，不能取代目視。最後把兩張場景圖與全部精靈逐張列進 `sw.js › ASSETS` 並提升 `VERSION`。少任何一張，都不能把新釣點標成完成。
