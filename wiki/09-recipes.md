...
6. **產生並接上完整正式美術。** 新釣點不能只靠 `scene.terrain`／`buildFish()`／`JUNK_MAPS` 看起來能玩就算完成；這三者必須保留做失敗備援，但正式交付還要同時具備：
   ...
   - 用 `tools/split-sprite-sheet.py` 依 id 名單去鍵色並縮進 96×56。預設幾何切格只適用於已確認每個主體完全留在格內的母版；只要頭尾、鰭鬚或特效可能跨格，**必須**從完整母版加 `--component-cells` 重切，不能先裁格再期待最大元件把缺掉的像素補回來。工具會清鍵色混邊、在縮小前處理任何色相的外光，並把成品最外圈統一成內側色衍生的暗描邊，**不是只針對紫色，也不是提高 `--key-tolerance` 把輪廓削掉**。必要時再用 `tools/flip-sprites.py` 修正魚頭方向。
   - 驗收必須同時通過：檔案數 = `loc.fish.length`、每張為 96×56 RGBA、同時有透明與不透明像素、安全留白、魚頭朝右。另把**全部正式成品**排在遊戲同款深色底做 **4 倍 contact sheet**，逐張看頭尾、鰭鬚與特效是否完整、是否混入鄰格碎片，最外圈不得有洋紅鍵色殘留，或青、藍、粉、紫等任何色相的外光；腳本檢查需確認近鍵色外緣與高彩度亮色外緣像素都是 0。合法鬚、蒸氣與裝飾可能分離，連通元件數只能輔助，不能取代目視。最後把兩張場景圖與全部精靈逐張列進 `sw.js › ASSETS` 並提升 `VERSION`。少任何一張美術、少任一輪檢查，都不能把新釣點標成完成。
