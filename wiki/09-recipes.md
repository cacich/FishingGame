...
2. 依稀有度插到對應的 `/* --- 稀有度 --- */` 區塊，欄位規格見 [07 §魚](07-data-schema.md#魚--locationfish)：
   ...
   ```
3. `id` 必須全域唯一，用釣點前綴（對照表見 [07 §釣點](07-data-schema.md#目前的十八個釣點)）。
...
| **魚王 cut-in** | `cutin.js › KING` | 一筆，key 是魚王的 `fish.id`。六行，見下方步驟 13 |
...
3. 選 `scene.terrain`。**不要沿用既有釣點的地形**——每個釣點一種是這個專案的原則，理由見 [06 §地形系統](06-pixel-engine.md#三之二--地形系統--terrain)。要新增地形照那一節的四個步驟做（記得補 `locThumb()` 的 `switch` case）。
   > 到第十八種為止，好用的辨識手段已經很密。**動手畫之前先看 [06 §十八種地形各自佔了哪一條辨識軸](06-pixel-engine.md#十八種地形各自佔了哪一條辨識軸)，挑一條還空著的。** 撞到既有的那一條，做出來就等於白做。
   >
...
5. 填魚。**七個階級都要有**（junk 3 / common 6 / good 5 / rare 4 / epic 2～3 / legend 2 / king 1），配額理由見 [07 §一個釣點該放幾條魚](07-data-schema.md#一個釣點該放幾條魚)。缺席的階級權重會消失，費率表會跟其他釣點不一致。
   ...
   - **`mult` 先隨便填**（同階級填一樣的數字就好），步驟 7 的換算腳本會全部覆蓋掉。
6. 設 `minBet`（**必須是 `FG.BETS` 裡真實存在的一格**），`unlock` 填 `{ free: true }`（現行釣點都免費，門檻靠 `minBet`）。**在 `FG.LOCATIONS` 裡的位置要跟 `minBet` 遞增一致**——那個順序同時是各處 UI 的顯示順序與玩家讀到的進程順序。插隊是安全的（所有參照都用 `loc.id`）。
   - 目前前十個釣點各佔 100 那一段的一格，後六個佔 1000 那一段，終點站停在 7,000（**刻意不用滿階梯頂**，這樣連最深的釣點都還有 4 檔可以加注）。
7. **跑換算腳本生 `mult`。** 這一步取代了舊版整套「填 `value` → 跑模擬 → 算校正係數」的流程。
   ...
   - 腳本會斷言替換筆數（目前 417），對不上就中止不寫檔；縮放 EV 要包含 `E[(len/avg)²]` 與非雜物閃光期望 1.06。
8. 拿掉 `comingSoon`，驗證：
...
9. **如果新釣點接在最後面**：檢查原本最後一名的 `desc` 有沒有寫死「最深」「最後」這類字眼。
10. **加那一整套周邊**（釣竿／餌料／裝備／家園裝飾），照上面的規則表。三件事別漏：
...
11. **檢查窄螢幕的介面**。把視窗縮到 320px，看圖鑑的釣點切換列、全螢幕釣點地圖庫（搜尋、四個分組、預覽、兩欄卡片）與**釣魚畫面的下注列**（[08 §會隨釣點數量成長的介面](08-ui-and-screens.md#會隨釣點數量成長的介面)）。`name` 建議 4 個字以內、`subtitle` 8 個字以內；地圖庫卡片會截斷過長副標，但大型預覽仍應能讀完整名稱。18 個釣點時圖鑑與下注列已驗證 body 寬度為 320px、沒有橫向溢出，後段釣點與 9,000／10,000 投注鈕可正常操作。
12. **檢查商店三個分頁每一列都有圖示**（一行 console 就能掃）：
    ```js
    ...
    ```
13. **給新魚王一筆 cut-in 資料**。`js/cutin.js › KING` 加一列，key 是魚王的 `fish.id`：
    ```js
...
📝 **要更新**：[07 資料規格](07-data-schema.md) 的釣點表（含 minBet 與可選檔數）、配色規則表與**釣竿／餌料／裝備／裝飾四張表**、[06 像素引擎](06-pixel-engine.md) 的地形表（若加了新地形）、[10 平衡調參](10-balance-tuning.md) 的波動度曲線與分布表、[03 經濟](03-economy.md) 的分布表、[12 名詞表](12-glossary.md) 的 id 前綴與 terrain 清單、[04 釣魚循環](04-fishing-loop.md) 的魚王 cut-in 分配表、[README](README.md) 的三十秒版本。
