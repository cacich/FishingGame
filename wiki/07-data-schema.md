...
```js
{
  ...
  unlock:      { free: true } | { chips: 120000 },   // 目前四個釣點都是 free
...
| id | 名稱 | castCost | unlock | 魚種數（含雜物） | 圖鑑分母 |
|---|---|---|---|---|---|
| `mist_lake` | 晨霧湖 | 400 | `free` | 24 | 21 |
| `sunset_fjord` | 落霞峽灣 | 1,100 | `free` | 24 | 21 |
| `frost_lake` | 幽藍冰湖 | 3,000 | `free` | 23 | 20 |
| `abyss` | 深淵海溝 | 12,000 | `free` | 23 | 20 |

「圖鑑分母」是 `codexProgress()` 算出來的數字，**雜物不列入**（見 [02 §圖鑑](02-state-and-save.md)）。四個釣點的圖鑑分母都刻意維持在 20 以上，讓每張圖鑑頁的收集進度條長度感覺一致。
...
> ⚠️ **`isUnlocked()` 對 `unlock.free` 是直接回 true，不看 `data.unlocked[]`。** 所以改成免費不需要遷移舊存檔（舊存檔的 `unlocked` 仍然只有 `['mist_lake']`，但四個釣點都進得去），改回收費也不會讓已付費的玩家重付。

`comingSoon` 目前**沒有任何釣點在用**（四個都已開放），欄位保留給未來新增的預告釣點。相關的 `unlockLoc()` 回傳值 `'soon'` 與兩處 UI 的「即將開放」分支都還在，加新的預告釣點時可以直接沿用。
...
`id` 命名慣例：釣點前綴 + 底線 + 名稱（`ml_` = mist_lake、`fj_` = sunset_fjord、`fr_` = frost_lake、`ab_` = abyss）。**id 一旦上線就不要改**，改了等於玩家的圖鑑紀錄消失。

### 一個釣點該放幾條魚

現行四個釣點都採同一套配額，**不是隨便長出來的**：
...
```js
{
  ...
  junkArt: 'boot',      // boot | can | weed | bottle | ice | bone（對應 pixel.js › JUNK_MAPS）
...
雜物美術可以跨釣點重複用（`weed` 同時當晨霧湖的水草、峽灣的廢棄漁網、冰湖的斷繩），**換的是 `name` 和 `desc`，不是圖**。這是刻意的：雜物是負面事件，玩家不會盯著看，投資新美術的邊際效益低——但每個釣點至少要有一件**專屬**的雜物來帶場景感（冰湖的 `ice`、深淵的 `bone`）。
