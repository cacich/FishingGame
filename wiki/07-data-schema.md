...
```js
{
  ...
  unlock:      { free: true } | { chips: 120000 },   // 目前十二個釣點都是 free
...
**陣列順序 = 進程順序 = 各處 UI 的顯示順序**，並且刻意跟 `castCost` 遞增一致（400 → 700 → 1,100 → 1,800 → 2,300 → 3,000 → 4,400 → 6,000 → 9,000 → 12,000 → 24,000 → 48,000）。十二個裡有五個是後來**插隊**進去的（煙雨蓮江補掉 3,000 → 12,000 的 4 倍跳躍；澄澈方池／潮落礁灘／懸瀑深潭／硫煙湯湖則把整條曲線的間距壓到 1.3～1.6 倍），世界樹根與黃沙冥河是接在尾巴的。插隊與接尾都是安全的：所有跨檔案的參照都用 `loc.id`（`data.loc`、`unlocked[]`、`bgCache`、魚的 id 前綴），沒有任何地方用索引記住釣點。唯一的索引是 `screen-codex.js › locIdx`，那是每次 `onShow()` 由 `data.loc` 重算的暫存值。
...
| `terrain` | key | **地形產生器**：`forest`（預設）`pond` `cliff` `shrine` `tidal` `ice` `waterfall` `karst` `caldera` `night` `yggdrasil` `desert`。見 [06 §地形系統](06-pixel-engine.md#三之二--地形系統--terrain) |
...
`farTree` / `midTree` / `nearTree` 是**由遠而近的三層剪影色**，名字沿用自最早的樹林實作；在其他地形裡它們分別代表崖壁層、冰川與冰脊、櫻花林等等。改名會動到十二個釣點的資料，**目前刻意不改**——知道這件事就好。
...
> ⚠️ **`rodIcon()` 的兩張色表必須跟這張表一樣長。** 它是 `cols[idx]` 直接取值（目前取 `% 17`，表也是 17 筆），**加第 18 支竿沒補配色就會拿到 undefined、圖示整個消失**。見 [11 §24](11-invariants-and-gotchas.md)。
...
**四件新裝飾裡有三件是會動的**（`shishi` 的竹筒週期性傾倒、`cascade` 的水柱橫紋往下捲、`onsen` 的蒸氣飄動）。這是刻意的：靜態的 `shishi` 跟一根斜靠在牆邊的棍子分不出來，靜態的 `cascade` 只是兩個盆子中間一條白線——**這類「靠動作定義的物件」，動畫不是加分項而是識別的必要條件**。
