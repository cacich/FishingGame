...
| `rod_derrick` | 曉日沉港 | 2,800,000 | 1.90 | 0.375 | 2.09 |
...
| `bait_rag` | 曉日沉港 | 5,000 | 5 | 1.96 | 0.00 | 1.23 | 3.14 |
...
| `eq_potline` | 1,800,000 | `rareMul: 1.19` | 曉日沉港 |

> **陣列刻意分成兩段**（通用五件在前、專屬十六件在後），所以整體 price **不是**遞增的。這沒有影響——`equipIcon()` 是用 `EQUIP_ART[e.id]` 查表，不吃 index。**新的專屬裝備直接加在陣列尾巴就好**，不用照 price 插隊。
...
| `beacon`（船首航標燈·曉日沉港主題） | 160,000 | 純裝飾 |
