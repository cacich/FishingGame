...
**這是一個固定 RTP 的博弈遊戲**：每一竿的期望回報被釘死在 `下注額 × 98%`，不管帶什麼裝備、在哪個釣點、下多少注（`state.js › rtpNorm()` 的恆等式，實測 24 萬組配置誤差 4.4e-16）。買釣竿／餌料／裝備／家園擴建花掉的錢會**扣 2% 之後進 Buffer 池，再由後續拋竿悄悄回吐**，所以「賠付 ÷（下注額＋商店消費）」的總體回收率也是 98%。
...
**十六個釣點全部開放且免費切換**：晨霧湖 → 澄澈方池 → 亂石急湍 → 落霞峽灣 → 琉璃珊瑚 → 宵櫻神域 → 潮落礁灘 → 幽藍冰湖 → 懸瀑深潭 → 煙雨蓮江 → 硫煙湯湖 → 深淵海溝 → 世界樹根 → 黃沙冥河 → 鐘乳暗穴 → 曉日沉港，各 23～24 種魚（圖鑑分母 20～21）、各一位魚王（**一王一種專屬體型**，第十六位是**全遊戲唯一一隻不是魚的魚王**——一隻章魚）、**各一種地形**（森林／人工池塘／溪流湍瀨／崖壁／珊瑚礁／神域／潮間帶／冰原／瀑布潭／峰林水鄉／火山口湖／夜海／世界樹／沙漠／鐘乳洞／棄港沉船）。進程門檻由**最低下注額**（100 → 7,000）承擔，不收解鎖費。

**釣點之間的差別是波動度，不是收益**（RTP 都是 98%）：晨霧湖普通魚賠 ×0.32、魚王 ×42；曉日沉港普通魚只賠 ×0.07，但魚王賠 ×102（下注 7,000 時是 71 萬籌碼）。深釣點不是比較好賺，是**賭得比較大**。
