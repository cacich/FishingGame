...
玩家**選一個下注額**（19 檔，100～10,000）＋消耗一份**餌料**拋竿 → 系統立刻抽好結果（[03](03-economy.md)）→ 播 4 段演出（[04](04-fishing-loop.md)）→ 玩家選擇**賣掉換籌碼**或**收藏進家園魚缸**。

**這是一個固定 RTP 的博弈遊戲**：每一竿的期望回報被釘死在 `下注額 × 98%`，不管帶什麼裝備、在哪個釣點、下多少注（`state.js › rtpNorm()` 的恆等式；目前完整矩陣為 299,943 組配置）。買釣竿／餌料／裝備／家園擴建花掉的錢會**扣 2% 之後進 Buffer 池，再由後續拋竿悄悄回吐**，所以「賠付 ÷（下注額＋商店消費）」的總體回收率也是 98%。

**裝備因此不會讓玩家變有錢，只會改變波動度**——`jackpotMul` 把期望值從小獎搬到大獎、`kingMul` 搬到魚王。進程的驅動力是**圖鑑收集、魚王 jackpot、解鎖更高的最低下注額**。

**釣到傳說以上會多插一段 cut-in 登場演出**（[04 §cut-in](04-fishing-loop.md#cut-in--傳說以上的登場演出)）。傳說一套通用，**十八位魚王各有自己的骨架、配色、粒子與登場音階**——但不是各刻一套動畫，而是「四種骨架 × 每王一筆參數」，所以加新魚王只要補六行資料。

**十八個釣點全部開放且免費切換**：晨霧湖 → 澄澈方池 → 亂石急湍 → 落霞峽灣 → 琉璃珊瑚 → 宵櫻神域 → 潮落礁灘 → 幽藍冰湖 → 懸瀑深潭 → 煙雨蓮江 → 硫煙湯湖 → 深淵海溝 → 世界樹根 → 黃沙冥河 → 鐘乳暗穴 → 曉日沉港 → 朱楓天守 → 雪見狐湯，各 23～24 種可釣項目（圖鑑分母 20～21）、各一位魚王、**各一種地形**。最後兩種地形是楓城水堀與雪見露天湯；進程門檻由**最低下注額**（100 → 9,000）承擔，不收解鎖費。

**釣點之間的差別是波動度，不是收益**（RTP 都是 98%）：滿裝時晨霧湖普通魚約賠 ×0.30、魚王 ×42；雪見狐湯普通魚約賠 ×0.06、魚王 ×96（下注 9,000 時約 87 萬籌碼）。深釣點不是比較好賺，是**賭得比較大**。

**每個釣點都配一整套專屬周邊**：主題釣竿、主題餌料、**釣點專屬裝備**、主題家園裝飾。加新釣點時這六樣要一起加——規則與（很重要的）「為什麼竿／餌可以通用但裝備必須綁釣點」見 [09 §新增一個釣點](09-recipes.md#新增一個釣點--一次要生出一整套)。

魚、圖示、房間與地形仍有 canvas 程序化備援；十八個釣點另配**點陣底圖**、橫式縮圖與 417 張可釣項目精靈，船上的木船／坐姿釣手／狗則使用十二格點陣待機序列。外部圖片載入失敗時會自動退回程序化繪製（[06](06-pixel-engine.md)）。
...
| `js/data.js` | 2550 | 所有資料表（18 個釣點、下注階梯、RTP 目標） | [07](07-data-schema.md) |
