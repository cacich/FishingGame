...
**釣到傳說以上會多插一段 cut-in 登場演出**（[04 §cut-in](04-fishing-loop.md#cut-in--傳說以上的登場演出)）。傳說一套通用，**十五位魚王各有自己的骨架、配色、粒子與登場音階**——但不是各刻一套動畫，而是「四種骨架 × 每王一筆參數」，所以加新魚王只要補六行資料。

**十五個釣點全部開放且免費切換**：晨霧湖 → 澄澈方池 → 亂石急湍 → 落霞峽灣 → 琉璃珊瑚 → 宵櫻神域 → 潮落礁灘 → 幽藍冰湖 → 懸瀑深潭 → 煙雨蓮江 → 硫煙湯湖 → 深淵海溝 → 世界樹根 → 黃沙冥河 → 鐘乳暗穴，各 23～24 種魚（圖鑑分母 20～21）、各一位魚王（**一王一種專屬體型**）、**各一種地形**（森林／人工池塘／溪流湍瀨／崖壁／珊瑚礁／神域／潮間帶／冰原／瀑布潭／峰林水鄉／火山口湖／夜海／世界樹／沙漠／鐘乳洞）。進程門檻由拋竿費（400 → 96,000）承擔，不收解鎖費。
...
| `js/pixel.js` | 3800 | 像素美術引擎（含地形系統，15 種地形） | [06](06-pixel-engine.md) |
| `js/data.js` | 1900 | 所有資料表（15 個釣點） | [07](07-data-schema.md) |
