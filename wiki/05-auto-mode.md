...
釣到傳說以上時會插一段 cut-in（[04 §cut-in](04-fishing-loop.md#cut-in--傳說以上的登場演出)），**它的長度不除以 `speed`**：極速（spd=4）跑到魚王一樣停 2.1 秒把演出播完。

沒有開關，也刻意不做成設定項——自動模式最常見的設定就是「釣到傳說就停」，那一刻的 cut-in 正好是整場的收尾。壓成 0.5 秒等於白做。傳說以上基礎機率約 1%，跑 500 局大概遇到五次，多花十秒不影響掛機效率。

`resolveAuto()` 完全不知道 cut-in 存在——它是在 `reel` 結束後才被呼叫的，cut-in 早就演完並拆掉了。新增停止條件、滾動紀錄、統計列都不受影響。
...
**1. 多留一筆 `castCost()` 的餘裕。** 只夠買餌但買完連一竿都拋不了的話，買了也沒意義。（`castCost()` 現在回傳的是**下注額**，所以在高注檔位這筆餘裕會很大——那是對的，餌買了拋不動一樣沒意義。）
...
**3. 買不起目前這款時會降級。** 從「買得起的最貴一款」往下挑。**理由**：玩家一換到高級餌、手頭又不寬裕，自動模式就會開場即停，體感上就是「這個功能壞了」。降級會讓 `jackpotMul` / `junkMul` 跟著下降，所以 toast 一定要講清楚換成哪一款（`自動補貨：活蝦 ×10（白布假餌買不起，改用這款）`）。

> **2026-08-06 之後這個分支幾乎跑不到了**：餌價從「25～5,000（倍增）」改成「20～120（等差）」，最貴的一包也只要 1,200 籌碼，而下注額最低就是 100。降級邏輯保留著，因為它是低注 + 低餘額時的最後一道保險。

連最便宜的 `bait_bread`（200 + 下注額）都買不起才回 `false` → 停止。
