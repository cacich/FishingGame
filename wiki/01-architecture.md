...
| `FG.locationPicker()` | 頂部地點按鈕的釣點選單彈窗。**開窗會把「目前」那張卡捲到清單中間**（十八個釣點、一次只看得到三張，不捲的話後段玩家每次都要從頭滑）。位置一定要用 `offsetTop` 算，見 [08 §.loc-list](08-ui-and-screens.md#二--釣點選單彈窗的清單--loc-list) 與 [11 §39](11-invariants-and-gotchas.md#39-在-modal-的開場動畫期間量-getboundingclientrect-會量到縮放後的值) |
