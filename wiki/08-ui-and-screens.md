...
```
#betRow    下注列：「下注」標籤 ＋ .seg.seg-scroll（21 檔）
#gearRow   釣竿 / 餌料 / 中獎機率 三顆 .gear-slot
#autoStats 自動模式進行中才顯示
.cast-row  拋竿鈕 ＋ 自動鈕
```

**下注列一定要用 `.seg-scroll`**：選項最多 21 檔（`FG.BETS`），用等分的 `.seg` 會把「10,000」以上壓到折行，跟圖鑑的釣點切換列是同一個坑（[11 §21](11-invariants-and-gotchas.md)）。按鈕維持固定高度與橫向捲動，新增 11,000／12,000 不會改變容器寬度。
...
目前 `rodIcon()` 以 `% 25` 搭配 25 組竿身／握把色；`eq_swordtassel` 與 `eq_dunhuanglamp` 也各有專屬 `EQUIP_ART`。家園端的 `DECO_ART` 同步包含 `swordscreen`／`apsarabanner`，避免資料存在但卡片只剩空白圖示。
...
> **第三個是釣魚畫面的下注列**（`#betRow`，21 檔）。它跟第一個用同一套解法（`.seg-scroll` ＋ `scrollEdges()`），細節寫在上方 [§釣魚 · screen-fishing.js 的 `#castBar`](#釣魚--screen-fishingjs-的-castbar)。它成長的來源不是釣點數而是 `FG.BETS` 的長度，但撞牆的機制一模一樣。
...
**20 個釣點的結構檢查（2026-08-19）**：圖鑑切換列與投注列仍由 `.seg-scroll` 承載，地圖庫仍以兩欄卡片加內部垂直捲動呈現；劍影寒潭／敦煌月泉與 11,000／12,000 沿用既有的固定高度按鈕，不新增會撐開 body 的容器。若改動按鈕 padding 或字級，仍須回到 320px 實機檢查。
...
卡片在 `< 420px` 為兩欄，`≥ 420px` 為三欄；320px 另縮短邊距與縮圖高度，短於 650px 的螢幕會壓縮標題、預覽高度並隱藏描述，優先保住至少一段可操作的網格。點卡片只更新預覽，**不會立刻切換地點**；玩家必須再按一次「前往 ○○」，避免密集網格誤觸。解鎖與 `comingSoon` 分支雖然現行二十站用不到，仍保留在預覽主按鈕上供未來資料使用。
...
顏色走兩個 CSS 變數 `--ci-key` / `--ci-accent`，由 `cutin.js` 從魚自己的 `colors.glow` / `colors.pattern` 寫進 style。**不要在 CSS 裡寫死稀有度顏色**——那樣二十位魚王會長得一樣。放射線與光帶用 `currentColor` 配 `color: var(--ci-key)`，因為 gradient 裡不能直接吃自訂屬性當色票。
