...
**對策**：RTP 是恆等式，**直接窮舉驗證**（目前 299,943 組配置）。要用蒙地卡羅交叉驗證公式有沒有抄錯，就拆成兩個低變異的量：**非魚王的賠付**與**魚王的出現頻率**。方法與實測數字見 [10 §驗證](10-balance-tuning.md#驗證驗恆等式不要驗統計量)。
...
- **每個釣點的每一個稀有度階級都要有魚。** 不是硬性規定（缺席只會讓該階權重消失，不會爆），但**費率表會因此跟其他釣點不一致**，玩家換釣點時看到機率跳動會覺得是 bug。現行十八個釣點都是 junk 3 / common 6 / good 5 / rare 4 / epic 2～3 / legend 2 / king 1，配額的理由見 [07 §一個釣點該放幾條魚](07-data-schema.md#一個釣點該放幾條魚)。
...
- **每個釣點的 `scene.terrain` 都要挑一條「還沒被佔走的辨識軸」。** 十八種地形已經涵蓋天際線形狀、單一巨物、橫帶、垂直筆畫、扇形、人造直線、陸地入水、透明水底、斜向水流、前景遮擋、無天空、跨水線物件、城郭階梯與冷暖雙光源。新地形動手前先看 [06 §十八種地形各自佔了哪一條辨識軸](06-pixel-engine.md#十八種地形各自佔了哪一條辨識軸)——**撞到既有的那一條，換地形就等於白做**。
