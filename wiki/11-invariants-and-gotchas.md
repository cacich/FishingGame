...
**對策**：兩張色表必須跟 `FG.RODS` 一樣長。目前是 **17 筆**並取 `% 17`，但**表的長度還是要跟著竿子數量一起長**——取餘數只是讓它不消失，配色會開始重複。（歷史：5 → 13 → 17，每次加釣點都要回來補。）
...
- **每個釣點的每一個稀有度階級都要有魚。** 不是硬性規定（缺席只會讓該階權重消失，不會爆），但**費率表會因此跟其他釣點不一致**，玩家換釣點時看到機率跳動會覺得是 bug。現行十二個釣點都是 junk 3 / common 6 / good 5 / rare 4 / epic 2～3 / legend 2 / king 1，配額的理由見 [07 §一個釣點該放幾條魚](07-data-schema.md#一個釣點該放幾條魚)。
...
- **每個釣點的 `scene.terrain` 都要挑一條「還沒被佔走的辨識軸」。** 十二種地形已經把天際線形狀、單一巨物、橫帶、垂直筆畫、扇形、人造直線、陸地入水、透明水底分完了。新地形動手前先看 [06 §十二種地形各自佔了哪一條辨識軸](06-pixel-engine.md#十二種地形各自佔了哪一條辨識軸)——**撞到既有的那一條，換地形就等於白做**（冰脊踩過一次，見 §16）。
